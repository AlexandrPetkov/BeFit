...
<head>
	...
	<title>All trainers on this site</title>
...
<body>
	...
	<div class="container">
		<ol class="breadcrumb">
			<li><a href="index.jsp">${home}</a></li>
			<li class="active">${ourTrainers}</li>
		</ol>
		<h3 class="thin text-center">${allTrainers_header}</h3>
				...
				<c:choose>
					...
					<div class="panel-body">
						...
						<div class="row top-margin">
							...
							<div class="col-sm-6">
								<p>${standing}: <strong>${user.experience_years}</strong> <small> ${years}</small></p>
							</div>
								<div class="col-sm-12">
									<p>${specialization}: <strong> ${user.specialization}</strong></p>
								</div>
							</div>
							<p class="text-left text-muted">${user.about}</p>
							<hr>
							<div class="row top-margin">
								<div class="col-sm-6">
									<label>${price}: <strong><a style="color: red">${user.price}</a></strong> <small>${perTraining}</small></label>
								</div>
										...
										<c:if test="${sessionScope.isTrainer eq false}">
											<c:if test="${not (sessionScope.user.idTrainers eq user.id)}">
												<form>
													...
													<button class="btn btn-action" type="submit">${employ}</button>
												</form>
											</c:if>
											<c:if test="${sessionScope.user.idTrainers eq user.id}">
												<form>
